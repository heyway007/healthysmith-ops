...
import { resolveEmployeeIdByLabel, currentEmployeeId } from "@/lib/employee-picker";
...
export async function createRequest(formData: FormData) {
  ...
  try {
    ...
    redirect(`/admin/purchase/requests/new?error=${encodeURIComponent(message)}`);
  }
...
export async function updateRequest(id: string, formData: FormData) {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "เกิดข้อผิดพลาด";
    redirect(`/admin/purchase/requests/${id}?error=${encodeURIComponent(message)}`);
  }
  revalidatePath("/admin/purchase/requests");
  redirect(`/admin/purchase/requests/${id}`);
}
...
export async function deleteRequest(id: string) {
  ...
  if (current?.status !== "draft") {
    redirect(`/admin/purchase/requests?error=${encodeURIComponent("ลบได้เฉพาะใบขอซื้อสถานะร่างเท่านั้น")}`);
  }
  const { error } = await supabase.from("purchase_requests").delete().eq("id", id);
  if (error) redirect(`/admin/purchase/requests?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/admin/purchase/requests");
}
...
export async function submitRequest(id: string) {
  ...
  const { error } = await supabase
    ...
    .eq("id", id)
    .eq("status", "draft");
  if (error) redirect(`/admin/purchase/requests/${id}?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/admin/purchase/requests");
  redirect(`/admin/purchase/requests/${id}`);
}
