...
import { resolveEmployeeIdByLabel } from "@/lib/employee-picker";
...
export async function createReceipt(formData: FormData) {
  ...
  try {
    ...
    redirect(`/admin/purchase/receipts/new?po=${purchase_order_id}&error=${encodeURIComponent(message)}`);
  }
  revalidatePath("/admin/purchase/receipts");
  revalidatePath("/admin/purchase/orders");
  redirect("/admin/purchase/receipts");
}
...
export async function cancelReceipt(id: string) {
  ...
  if (!receipt || receipt.status !== "confirmed") {
    redirect(
      `/admin/purchase/receipts?error=${encodeURIComponent("ยกเลิกได้เฉพาะใบรับสินค้าที่ยืนยันแล้วเท่านั้น")}`
    );
    return;
  }
