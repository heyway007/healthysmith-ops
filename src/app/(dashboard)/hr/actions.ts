...
export async function createEmployee(formData: FormData) {
  ...
  try {
    ...
    redirect(`/hr/new?error=${encodeURIComponent(message)}`);
  }
  revalidatePath("/hr");
  redirect("/hr");
}
...
export async function updateEmployee(id: string, formData: FormData) {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "เกิดข้อผิดพลาด";
    redirect(`/hr/${id}?error=${encodeURIComponent(message)}`);
  }
  revalidatePath("/hr");
  redirect("/hr");
}
...
export async function deleteEmployee(id: string) {
  ...
  if (error) redirect(`/hr?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/hr");
}
