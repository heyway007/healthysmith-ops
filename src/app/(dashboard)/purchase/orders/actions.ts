...
export async function createOrder(formData: FormData) {
  ...
  try {
    ...
    redirect(`/purchase/orders/new?error=${encodeURIComponent(message)}`);
  }
...
export async function updateOrder(id: string, formData: FormData) {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "เกิดข้อผิดพลาด";
    redirect(`/purchase/orders/${id}?error=${encodeURIComponent(message)}`);
  }
  revalidatePath("/purchase/orders");
...
export async function deleteOrder(id: string) {
  ...
  if (current?.status !== "draft") {
    redirect(`/purchase/orders?error=${encodeURIComponent("ลบได้เฉพาะใบสั่งซื้อสถานะร่างเท่านั้น")}`);
  }
  const { error } = await supabase.from("purchase_orders").delete().eq("id", id);
  if (error) redirect(`/purchase/orders?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/purchase/orders");
}
