...
import { currentEmployeeId } from "../employee-picker";
...
export async function createBill(formData: FormData) {
  ...
  try {
    ...
    redirect(`/purchase/bills/new?error=${encodeURIComponent(message)}`);
  }
  revalidatePath("/purchase/bills");
  revalidatePath("/purchase/payables");
...
export async function updateBill(id: string, formData: FormData) {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "เกิดข้อผิดพลาด";
    redirect(`/purchase/bills/${id}?error=${encodeURIComponent(message)}`);
  }
  revalidatePath("/purchase/bills");
  revalidatePath("/purchase/payables");
  redirect(`/purchase/bills/${id}`);
}
...
export async function deleteBill(id: string) {
  ...
  if (current?.status !== "draft") {
    redirect(`/purchase/bills?error=${encodeURIComponent("ลบได้เฉพาะบิลสถานะร่างเท่านั้น")}`);
  }
  const { error } = await supabase.from("purchase_bills").delete().eq("id", id);
  if (error) redirect(`/purchase/bills?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/purchase/bills");
}
...
export async function submitBill(id: string) {
  ...
  const { error } = await supabase
    ...
    .eq("id", id)
    .eq("status", "draft");
  if (error) redirect(`/purchase/bills/${id}?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/purchase/bills");
  redirect(`/purchase/bills/${id}`);
}
