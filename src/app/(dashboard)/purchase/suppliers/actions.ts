...
export async function createSupplier(formData: FormData) {
  ...
  if (error) redirect(`/purchase/suppliers/new?error=${encodeURIComponent(error.message)}`);
...
export async function updateSupplier(id: string, formData: FormData) {
  ...
  if (error) redirect(`/purchase/suppliers/${id}?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/purchase/suppliers");
  redirect("/purchase/suppliers");
}
...
export async function deleteSupplier(id: string) {
  ...
  if (error) redirect(`/purchase/suppliers?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/purchase/suppliers");
}
